...
func TestArraySupport(t *testing.T) {
	...
	product := TestProductWithArrays{
		ID:         1,
		Name:       "Test Product",
		Categories: duckdb.StringArray{"electronics", "computers", "laptops"},
		Scores:     duckdb.FloatArray{4.5, 4.8, 4.2},
		ViewCounts: duckdb.IntArray{100, 250, 75},
		CreatedAt:  now,
	}
	...
	retrieved.Categories = duckdb.StringArray{"electronics", "computers", "laptops", "gaming"}
...
func TestArrayValuerScanner(t *testing.T) {
	...
	t.Run("StringArray", func(t *testing.T) {
		arr := duckdb.StringArray{"hello", "world", "test"}
		...
		// Test Scan()
		var scanned duckdb.StringArray
	...
	t.Run("FloatArray", func(t *testing.T) {
		arr := duckdb.FloatArray{1.5, 2.7, 3.14}
...
func TestArrayEdgeCases(t *testing.T) {
	...
	t.Run("EmptyArrays", func(t *testing.T) {
		emptyStr := duckdb.StringArray{}
		val, err := emptyStr.Value()
		assert.NoError(t, err)
		assert.Equal(t, "[]", val)

		emptyInt := duckdb.IntArray{}
		val, err = emptyInt.Value()
		assert.NoError(t, err)
		assert.Equal(t, "[]", val)

		emptyFloat := duckdb.FloatArray{}
	...
	t.Run("NilArrays", func(t *testing.T) {
		var nilStr duckdb.StringArray
		val, err := nilStr.Value()
		assert.NoError(t, err)
		assert.Nil(t, val)

		var nilInt duckdb.IntArray
		val, err = nilInt.Value()
		assert.NoError(t, err)
		assert.Nil(t, val)

		var nilFloat duckdb.FloatArray
	...
	t.Run("StringEscaping", func(t *testing.T) {
		arr := duckdb.StringArray{"hello's", "world\"test", "normal"}
		val, err := arr.Value()
		assert.NoError(t, err)
